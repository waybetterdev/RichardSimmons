...
module.exports = (robot) ->
  ...
  robot.respond /current time( in)? (.*)/i, (msg) ->
    url = 'http://api.timezonedb.com/time-zones'
    query = msg.match[2]
    unless process.env["HUBOT_TIMEZONEDB_API_KEY"]
      msg.send 'Please, set HUBOT_CURRENT_TIME_KEY environment variable'
      return

    msg.http(url)
    .query(format: 'json', zone: query, key: process.env["HUBOT_TIMEZONEDB_API_KEY"])
    .get() (err, res, body) ->
        if err
